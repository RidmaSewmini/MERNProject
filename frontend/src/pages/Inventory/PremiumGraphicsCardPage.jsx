...
const graphicsCards = [
  ...
  {
    id: 2,
    image: "src/Asset/images/gpu2.png",
  ...
  {
    id: 3,
    image: "src/Asset/images/gpu3.png",
  ...
  {
    id: 4,
    image: "src/Asset/images/gpu4.png",
...
const memoryOptions = ["All", "8GB", "12GB", "16GB", "24GB"];
const availabilityOptions = ["All", "In Stock", "Out of Stock", "Pre-Order"];

const PremiumGraphicsCards = () => {
  ...
  const filteredCards = graphicsCards.filter(card => {
    ...
    const matchesPrice = card.priceValue >= priceRange[0] && card.priceValue <= priceRange[1];
    const matchesBrand = selectedBrand === "All" || card.brand === selectedBrand;
    const matchesMemory = selectedMemory === "All" || card.memory === selectedMemory;
    const matchesAvailability = selectedAvailability === "All" || card.availability === selectedAvailability;
    return matchesSearch && matchesPrice && matchesBrand && matchesMemory && matchesAvailability;
  });
    ...
    <div className="flex flex-col min-h-screen font-titillium" style={{ backgroundColor: "#141313", color: "white" }}>
      ...
      <div style={{ padding: "20px", maxWidth: "1400px", margin: "0 auto", width: "100%" }}>
          ...
          {showFilters && (
            <div style={{ backgroundColor: "#2a2a2a", padding: "20px", borderRadius: "10px", display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
                ...
                <div style={{ flex: "1", minWidth: "200px" }}>
                  <h3 style={{ marginBottom: "10px", color: "#ff9800" }}>MEMORY</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {memoryOptions.map(mem => (
                      <label key={mem} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <input
                          type="radio"
                          name="memory"
                          value={mem}
                          checked={selectedMemory === mem}
                          onChange={() => setSelectedMemory(mem)}
                        />
                        {mem}
                      </label>
                    ))}
        ...
        <div style={{ marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p>{sortedCards.length} Graphics Cards Found</p>
        ...
        {sortedCards.length > 0 ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "25px", justifyItems: "center" }}>
            {sortedCards.map((card) => (
              ...
              >
                ...
                />
                <div style={{ fontSize: "18px", fontWeight: "bold", marginBottom: "10px", flexGrow: 1 }}>{card.title}</div>
                <div style={{ color: "orange", margin: "5px 0" }}>- {card.memory} -</div>
                <div style={{ fontSize: "22px", fontWeight: "bold", margin: "10px 0", color: "#ff9800" }}>{card.price}</div>
                <div style={{ ...getAvailabilityStyle(card.availability), borderRadius: "5px", padding: "5px 15px", fontWeight: "bold", marginTop: "10px", display: "inline-block", alignSelf: "center" }}>
                  {card.availability}
                </div>
          ...
          <div style={{ textAlign: "center", padding: "40px", color: "#888" }}>
            <h2>No graphics cards found matching your criteria</h2>
...
export default PremiumGraphicsCards;
