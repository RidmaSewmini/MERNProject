...
// Sample Gaming Peripherals data
const peripherals = [
  {
    id: 1,
    image: "src/Asset/images/keyboard1.png",
    title: "Razer Huntsman V2 Keyboard",
    price: "45,000 LKR",
    alt: "Razer Huntsman Keyboard",
    brand: "Razer",
    type: "Keyboard",
    connectivity: "Wired",
  ...
  {
    id: 2,
    image: "src/Asset/images/mouse1.png",
  ...
  {
    id: 3,
    image: "src/Asset/images/headset1.png",
    title: "SteelSeries Arctis 7 Headset",
    price: "35,000 LKR",
    alt: "SteelSeries Arctis Headset",
    brand: "SteelSeries",
    type: "Headset",
  ...
  {
    id: 4,
    image: "src/Asset/images/controller1.png",
...
const brands = ["All", "Razer", "Logitech", "SteelSeries", "Microsoft"];
const types = ["All", "Keyboard", "Mouse", "Headset", "Controller"];
const connectivityOptions = ["All", "Wired", "Wireless"];
...
const GamingPeripherals = () => {
  ...
  const filteredPeripherals = peripherals.filter(item => {
    const matchesSearch = item.title.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesPrice = item.priceValue >= priceRange[0] && item.priceValue <= priceRange[1];
    const matchesBrand = selectedBrand === "All" || item.brand === selectedBrand;
    const matchesType = selectedType === "All" || item.type === selectedType;
    const matchesConnectivity = selectedConnectivity === "All" || item.connectivity === selectedConnectivity;
    const matchesAvailability = selectedAvailability === "All" || item.availability === selectedAvailability;

    return matchesSearch && matchesPrice && matchesBrand && matchesType && matchesConnectivity && matchesAvailability;
  });

  const sortedPeripherals = [...filteredPeripherals].sort((a, b) => {
    switch(sortOption) {
    ...
    <div className="flex flex-col min-h-screen font-titillium" style={{ backgroundColor: "#141313", color: "white" }}>
      ...
      <div style={{ padding: "20px", maxWidth: "1400px", margin: "0 auto", width: "100%" }}>
          ...
          {showFilters && (
            <div style={{ backgroundColor: "#2a2a2a", padding: "20px", borderRadius: "10px", display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
                ...
                <div style={{ flex: "1", minWidth: "200px" }}>
                  <h3 style={{ marginBottom: "10px", color: "#ff9800" }}>TYPE</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {types.map(type => (
                      <label key={type} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <input type="radio" name="type" value={type} checked={selectedType === type} onChange={() => setSelectedType(type)} />
                        {type}
                      </label>
                    ))}
                  </div>
                ...
                <div style={{ flex: "1", minWidth: "200px" }}>
                  <h3 style={{ marginBottom: "10px", color: "#ff9800" }}>CONNECTIVITY</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {connectivityOptions.map(conn => (
                      <label key={conn} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <input type="radio" name="connectivity" value={conn} checked={selectedConnectivity === conn} onChange={() => setSelectedConnectivity(conn)} />
                        {conn}
                      </label>
                    ))}
                  </div>
        ...
        <div style={{ marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p>{sortedPeripherals.length} Peripherals Found</p>
        ...
        {sortedPeripherals.length > 0 ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "25px", justifyItems: "center" }}>
            {sortedPeripherals.map((item) => (
              ...
              >
                ...
                <div style={{ color: "orange", margin: "5px 0" }}>- {item.type} / {item.connectivity} -</div>
          ...
          <div style={{ textAlign: "center", padding: "40px", color: "#888" }}>
            <h2>No peripherals found matching your criteria</h2>
